...
export module mzia.util.std_extension.io;
...
// to make mzia::meta::is_std_ostream_interactable<...>::value evaluate to true, the candidate overload must be already defined above it
...
export template <std::ranges::input_range Range, typename = std::enable_if_t<!::mzia::meta::is_std_ostream_interactable_v<Range>>>
auto& operator <<(std::ostream& ostr, Range&& range)
{
  ::mzia::print(ostr, std::forward<Range>(range));
  return ostr;
}
