...
[[nodiscard]] std::string rand_big_integer_impl(std::size_t length) {
  ...
  if constexpr (AllowNegative)
    if (rand_number(0uz, 1uz))
      data += '-';

  data += radix_digits<10>[rand_number(1uz, 9uz)];
...
//     else
//       data.add_edge(p[rand_number(0uz, v - 1)], p[v]);
//   }
...
//     else
//       data.add_edge(p[rand_number(0uz, v - 1)], p[v], w_dist(mt19937_engine));
//   }
